...
/*
  Catmull-Clark
*/
const generateLookupsCat = (sourceVertices, sourceFaces, oldVerticesRelationships, sourceEdges) => {
...
const processEdgeCat = (v1, v2, sourceVertices, sourceEdges, currentFace, metaVertices) => {
...
/*
  loop
*/
const generateLookupsLoop = (oldVertices, oldFaces, metaVertices, sourceEges) => {
  // for every vertex, add a new object w/ array of objects.
...
const processEdgeLoop = (v1, v2, oldVertices, sourceEdges, currentFace, metaVertices) => {
  const vertexIndexA = Math.min(v1, v2);
  const vertexIndexB = Math.max(v1, v2);
  const key = vertexIndexA + "_" + vertexIndexB;
  let edge;
